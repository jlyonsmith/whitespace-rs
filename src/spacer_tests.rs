...
fn test_write_new_file_round_down() {
  ...
  let line_info = write_new_bols(&mut input, &mut output, BeginningOfLine::Tabs, 2, true).unwrap();
...
fn test_write_new_file() {
  let mut input = " a\n   x\n    \n".as_bytes();
  let mut output = Vec::new();
  let line_info = write_new_bols(&mut input, &mut output, BeginningOfLine::Tabs, 2, false).unwrap();
